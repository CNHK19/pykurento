...
class KurentoTransportException(Exception):
    ...
    def __str__(self):
        return "%s - %s" % (str(self.message), simplejson.dumps(self.response))
...
class KurentoTransport(object):
    ...
    def _on_message(self, message):
        ...
        if 'method' in resp:
            ...
            elif (resp['method'] == 'onEvent'
                  and 'params' in resp
                  and 'value' in resp['params']
                  and resp['params']['value']
                  and 'type' in resp['params']['value']
                  and resp['params']['value']['type'] == 'IceCandidateFound'
                  and 'data' in resp['params']['value']
                  and 'candidate' in resp['params']['value']['data']):
                candidate = resp['params']['value']['data']['candidate']
                self.ice_candidates.append(dict(candidate=candidate))
                  ...
                  and resp['params']['value']
                  and 'type' in resp['params']['value']
                  and resp['params']['value']['type'] == 'OnIceCandidate'
                  and 'data' in resp['params']['value']
                  and 'candidate' in resp['params']['value']['data']):

                candidate = resp['params']['value']['data']['candidate']
                source = resp['params']['value']['data']['source']
                self.ice_candidates.append(dict(candidate=candidate, source=source))
    ...
    def _rpc(self, rpc_type, **args):
        ...
        logger.debug("sending message:  %s" % simplejson.dumps(request))
        self.ws.send(simplejson.dumps(request))
